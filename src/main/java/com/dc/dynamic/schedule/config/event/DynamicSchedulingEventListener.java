    ...
    public void updateSchedule(Object object){
        if (object instanceof  DynamicScheduleEvent) {
            ...
            if(scheduleEvent.getCancel()) {
                ...
                //判断新的和旧的cron 表达式是否相同,但是前提是上一次开启的
                if ( (!dcSchedulingRunnable.getStatus()) || (!dcSchedulingRunnable.getCronExpression().equals(scheduleEvent.getCronExpression()))) {
                    dcSchedulingRunnable.setCronExpression(scheduleEvent.getCronExpression());
                    dcSchedulingRunnable.setStatus(scheduleEvent.getCancel());
